...
- **文件流返回体模拟**
  - REST 返回体支持 `TEXT` / `FILE` 两种类型，可上传二进制文件作为 Mock 响应。
  - 文件响应保留原始文件名、下载文件名、大小、Content-Type 等元数据。
  - 删除或替换文件返回体时只清理不再被引用的文件，避免误删仍被其他返回体使用的文件。
  - 增加孤儿文件扫描与手动清理能力，便于维护长期测试产生的文件。
- **SOAP 条件返回**
  - SOAP 接口的每个 Operation 支持独立配置多个返回体。
  - 多启用返回体时可配置响应规则，按顺序匹配命中返回，未命中走无规则兜底。
  - SOAP Body 条件支持 XML 元素路径匹配，例如 `GetUserRequest.userId`。
  - SOAP 条件匹配按 Operation 隔离，避免不同 Operation 的返回体互相串用。

### 修复

- **接口列表最近调用时间格式**：悬浮提示统一为 `yyyy-MM-dd HH:mm:ss`。
- **文件返回体编辑回显**：修复点击编辑文件类型 Mock 接口时，已上传文件信息找不到的问题。
- **接口分组名称显示**：修复“所有接口”视图中分组列显示为 `-` 的问题。
- **SOAP 响应规则回显**：修复编辑 SOAP 返回体时 `conditions` 未回填，导致已有条件显示为“无规则”的问题。

### 文档

- README 补充 SOAP Operation 多返回体、条件规则、无规则兜底和 XML Body 路径匹配说明。
- README 修正多场景响应、WSDL 托管地址等功能描述，使其与 v1.4.5 能力一致。
